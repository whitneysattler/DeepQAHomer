...
class Chatbot:
    ...
    def mainTestInteractive(self, sess):
        ...
        encodeModel = skipthoughts.load_model()
        encoder = skipthoughts.Encoder(encodeModel)
        decoder = tools.load_model()
        cornyVec = np.load("../skip-thoughts/cornellFlavor.npy")
        homeyVec = np.load("../skip-thoughts/homerLong.npy")
        ...
        while True:
            ...
            if not answer:
                ...
                continue  # Back to the beginning, try again
            botAnswer = self.textData.sequence2str(answer, clean=True) #botAnswer is the human-readable output of chatbot
            vectorAnswer = encoder.encode([botAnswer]) #vectorAnswer is the converted vector-embedded response
            skipAnswer = tools.run_sampler(decoder,vectorAnswer, beam_width=1, stochastic=False, use_unk=False)

            newVec = vectorAnswer - cornyVec + homeyVec
            homerAnswer = tools.run_sampler(decoder, newVec, beam_width=1, stochastic=False, use_unk=False) #anwer is the decoded version of the vectorAnswer

            print('{}{}'.format("DeepQA Answer: ", botAnswer))
            print('{}{}'.format("Skip-Thought Answer: ", skipAnswer[0]))
            print('{}{}'.format("Homer Answer: ", homerAnswer[0]))
